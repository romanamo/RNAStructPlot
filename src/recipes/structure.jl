...
@recipe StructurePlot begin
    "Layout options (:polygonal, :modified)"
    layout = :polygonal #:modified
...
function Makie.plot!(ssg::StructurePlot{<:Tuple{RNABaseGraph}})
    ...
    # calculate layout
    map!(ssg.attributes, [:converted_1, :layout], [:result]) do rnabase, layout
        if layout == :polygonal
            return (layoutpolygonal(rnabase),)
        elseif layout == :modified
            return (layoutmodified(rnabase),)
        else
            error("Not a valid layout option. Valid options are: :polygonal, :modified")
        end
    end
    ...
    # calculate edges
    calc_edges_input = [:converted_1, :result]
    calc_edges_output = [:seqx, :seqy, :aux, :auy, :cgx, :cgy, :gux, :guy, :otherx, :othery]

    map!(ssg.attributes, calc_edges_input, calc_edges_output) do rnabase, result
        ...
        for edge in edges(rnabase.graph)
            srcx, srcy = result.coords[src(edge)]
            dstx, dsty = result.coords[dst(edge)]
